        ...
        with open(os.path.join(caption_root, classes_filename)) as f:
            ...
            for line in lines:
                ...
                for filename in filenames:
                    datum = pickle.load(open(os.path.join(caption_root, cls, filename),"rb"))
                    
                    raw_desc = datum['char'].numpy()
                    desc, len_desc = self._get_word_vectors(raw_desc, word_embedding)
                    # ./datasets/img/Classic_Knit_Cardigan/img_00000030.jpg
                    if os.path.join(img_root, datum['img']).split('/')[-2] != 'Striped_A-line_Dress':
                      # print(os.path.join(img_root, datum['img']))
                      # print(datum['img'])
                      # print(caption_root)
                      # print(cls)
                      # print(classes_filename)
                      # print(os.path.join(caption_root, classes_filename))
                      # # print(img_root)
                      # # print(filename)
                      # abc
                      output.append({
